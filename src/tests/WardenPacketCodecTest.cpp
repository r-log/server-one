...
warden::ModuleProfile const* Windows8606Profile()
{
    static warden::WardenModuleCatalog const catalog;
    return catalog.Find(8606, "Win");
}
...
{
    warden::ModuleProfile const* profile = Windows8606Profile();
...
TEST(WardenPacket_encodes_exact_8606_module_initialization)
{
    warden::ModuleProfile const* profile = Windows8606Profile();
    REQUIRE(profile != nullptr);
...
TEST(WardenPacket_encodes_exact_8606_timing_and_mem_request)
{
    warden::ModuleProfile const* profile = Windows8606Profile();
    REQUIRE(profile != nullptr);
...
TEST(WardenPacket_encodes_and_decodes_exact_mem_only_confirmation)
{
    ...
    CHECK_HEX(encoded.data(), encoded.size(),
        "0207576f572e657865008c01100c2d00287f");
...
TEST(WardenPacket_decodes_exact_8606_timing_mem_result_in_plan_order)
{
...
TEST(WardenPacket_repeats_mpq_and_lua_definitions_with_shared_strings)
{
    ...
    warden::WardenCheckProfile const* checks =
        catalog.Find(8606, "Win", "enUS");
    warden::ModuleProfile const* module = Windows8606Profile();
...
TEST(WardenPacket_enforces_client_transport_result_body_budget)
{
    ...
    static_assert(proto::MAX_CLIENT_PACKET_SIZE - sizeof(uint32) -
        ...
        "TBC Warden CHECK_RESULT body limit must remain exact");
