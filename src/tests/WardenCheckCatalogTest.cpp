...
TEST(WardenCheckCatalog_decodes_and_selects_eight_exact_8606_profiles)
{
    ...
    {{
        {"enUS", "d65d59d2e57792a13e8edf78a574b8f81d0d3cf0",
            "4f6b6179"},
        {"enGB", "d65d59d2e57792a13e8edf78a574b8f81d0d3cf0",
            "4f6b6179"},
        {"deDE", "002ecf12a5b297dace7955e5971e4f68bf8a8dab",
            "4f4b"},
        {"esES", "dbe1dbade50f3a616b48f8909d3c8ef75c0d9220",
            "41636570746172"},
        {"frFR", "c7c50539f79bd77e28a6328cb13fcffa596f1f7b",
            "4f4b"},
        {"koKR", "7ad1756c8a4bd449698a98fdc58775ee2f0fbd6e",
            "ed9995ec9db8"},
    ...
    for (ExpectedProfile const& expected : expectedProfiles)
    {
        warden::WardenCheckProfile const* profile =
            catalog.Find(8606, "Win", expected.locale);
        ...
        CHECK_STR(mem.moduleName.c_str(), "WoW.exe");
        CHECK_EQ(mem.addressOrRva, uint32(0x002D0C10));
    ...
    CHECK(catalog.Find(8606, "Win", "esMX") == nullptr);
    CHECK(catalog.Find(8606, "OSX", "zhCN") == nullptr);
}
...
TEST(WardenCheckCatalog_preserves_embedded_zero_bytes)
{
    ...
    rows.resize(2);
    rows[1] = warden::test::MakeRow(8606, "656E5553", 9001,
...
TEST(WardenCheckCatalog_enforces_complete_profiles_and_atomic_build)
{
    ...
    REQUIRE(BuildRows(rows, observationOnly) ==
        warden::CheckCatalogValidation::Valid);
    warden::WardenCheckProfile const* profile =
        observationOnly.Find(8606, "Win", "enUS");
    REQUIRE(profile != nullptr);
    CHECK(!profile->hasActionableChecks);
