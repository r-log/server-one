...
{
    return TestChecks(8606, "enUS", {65536});
}

std::vector<warden::WardenCheckDefinition> TestMemChecks()
{
    return TestChecks(8606, "enUS", {65536, 3});
}
...
struct Harness
{
        ...
        : peer(TestSessionKey()), sendSucceeds(allowSend)
    {
        if (!EnsureTestCatalogPublished())
            return;
        server = warden::WardenManager::Instance().Create(8606, "Win",
...
{
    return FromHex(
        "031400AB38846D010002007079250080402500004E250090522500"
        "0308002D138DA10400000072300001"
        "03080087A16FB40101005098340001");
}
...
bool StartTimingMpqLuaMemCheck(Harness& harness)
{
    ...
    return harness.peer.DecryptServer(harness.sent.back()) == FromHex(
        "021B444246696C6573436C69656E745C41"
        "7265615461626C652E646263044F4B4159"
        "07576F572E6578650028E701F4028C03"
        "100C2D00287F");
}
...
bool StartTimingMemCheck(Harness& harness)
{
    ...
    return harness.peer.DecryptServer(harness.sent.back()) == FromHex(
        "0207576F572E65786500288C01100C2D00287F");
}
}
...
TEST(WardenManager_creation_is_inert_for_exact_8606_and_rejects_unknown_ones)
{
    ...
    std::unique_ptr<warden::WardenServer> supported =
        warden::WardenManager::Instance().Create(8606, "Win", "enUS",
            TestSessionKey(), send);
    ...
    CHECK(warden::WardenManager::Instance().Create(8606, "OSX", "enUS",
...
TEST(WardenManager_publishes_one_immutable_check_catalogue_snapshot)
{
    ...
    warden::WardenCheckProfile const* selected =
        manager.FindCheckProfile(8606, "Win", "enUS");
    REQUIRE(selected != nullptr);
    CHECK_EQ(selected->checks.size(), size_t(4));
    CHECK(!manager.PublishCheckCatalog(empty));
    CHECK(!manager.PublishCheckCatalog(second));
    CHECK(manager.FindCheckProfile(8606, "Win", "enUS") == selected);
}
...
TEST(WardenManager_selects_content_checks_only_for_the_exact_locale)
{
    ...
    Harness esMX(ManagerLocale{"esMX"}, true, observe);
    REQUIRE(ReachModuleReady(esMX));
    esMX.server->Update(true, 60000);
    CHECK(esMX.server->GetState() == warden::WardenState::ModuleReady);
    CHECK_EQ(esMX.sent.size(), size_t(3));
    CHECK(esMX.evidenceEvents.empty());
}
...
TEST(WardenManager_enforcing_modes_require_exact_check_profiles)
{
    ...
    std::array<char const*, 8> const locales =
            ...
            warden::WardenEnforcementMode::KickAndBan})
    {
        ...
        {
            CHECK(warden::WardenManager::Instance().Create(8606,
                "Win", locale, TestSessionKey(), send,
                options) != nullptr);
        }
        CHECK(warden::WardenManager::Instance().Create(8606,
            "Win", "esMX", TestSessionKey(), send, options) == nullptr);
    }
}
...
TEST(WardenManager_observe_mode_allows_missing_check_profile)
{
    ...
    CHECK(warden::WardenManager::Instance().Create(8606, "Win", "esMX",
...
{
    REQUIRE(EnsureTestCatalogPublished());
    for (char const* locale : {"enUS", "enGB", "deDE", "esES", "frFR",
             "koKR", "ruRU", "zhCN"})
        CHECK(warden::IsWardenEnforcementProfile(8606, "Win", locale));

    CHECK(!warden::IsWardenEnforcementProfile(8606, "Win", "esMX"));
    CHECK(!warden::IsWardenEnforcementProfile(8606, "OSX", "enUS"));
    CHECK(!warden::IsWardenEnforcementProfile(12340, "Win", "enUS"));
    CHECK(!warden::IsWardenEnforcementProfile(8606, "Win", ""));
}
...
{
    Harness harness(true, TestChecks(8606, "enUS", {65536, 1}));
    REQUIRE(StartTimingMpqCheck(harness));

    harness.SendClient(FromHex(
        "021A00B8D19BD3010403020100D65D59D2"
        "E57792A13E8EDF78A574B8F81D0D3CF0"));
    CHECK(harness.server->GetState() == warden::WardenState::ModuleReady);
    CHECK(harness.server->GetFailure() == warden::WardenFailure::None);
    REQUIRE(harness.evidenceBatches.size() == 1u);
    CHECK(harness.evidenceBatches[0].purpose ==
...
{
    Harness unavailable(true, TestChecks(8606, "enUS", {65536, 1}));
    ...
    Harness mismatch(true, TestChecks(8606, "enUS", {65536, 1}));
    ...
    Harness bothNegative(true, TestChecks(8606, "enUS", {65536, 1}));
...
{
    Harness harness(true, TestChecks(8606, "enUS", {65536, 1, 2}));
    REQUIRE(StartTimingMpqLuaCheck(harness));

    harness.SendClient(FromHex(
        "022000755A512D010403020100D65D59D2"
        "E57792A13E8EDF78A574B8F81D0D3CF0"
        "00044F6B6179"));
    CHECK(harness.server->GetState() == warden::WardenState::ModuleReady);
    CHECK(harness.server->GetFailure() == warden::WardenFailure::None);
...
TEST(WardenServer_all_eight_locale_content_vectors_report_ordered_matches)
{
    ...
    {
        {"enUS", "022000755A512D010403020100D65D59D2E57792A13E8EDF78A574B8F81D0D3CF000044F6B6179"},
        {"enGB", "022000755A512D010403020100D65D59D2E57792A13E8EDF78A574B8F81D0D3CF000044F6B6179"},
        {"deDE", "021E0093C29AA1010403020100002ECF12A5B297DACE7955E5971E4F68BF8A8DAB00024F4B"},
        {"esES", "022300F16E957A010403020100DBE1DBADE50F3A616B48F8909D3C8EF75C0D9220000741636570746172"},
    ...
    {
        REQUIRE(TestCheckCatalog().Find(8606, "Win",
            vector.locale) != nullptr);

        Harness harness(true, TestChecks(8606, vector.locale,
            {65536, 1, 2}));
        REQUIRE(StartTimingMpqLuaCheck(harness));
...
TEST(WardenServer_valid_lua_negatives_are_observation_only)
{
    Harness unavailable(true,
        TestChecks(8606, "enUS", {65536, 1, 2}));
    REQUIRE(StartTimingMpqLuaCheck(unavailable));
    unavailable.SendClient(FromHex(
        "021B000F4297AC010403020100D65D59D2"
        "E57792A13E8EDF78A574B8F81D0D3CF001"));
    CHECK(unavailable.server->GetState() == warden::WardenState::ModuleReady);
    CHECK(unavailable.server->GetFailure() == warden::WardenFailure::None);
    ...
    Harness mismatch(true, TestChecks(8606, "enUS", {65536, 1, 2}));
    REQUIRE(StartTimingMpqLuaCheck(mismatch));
    mismatch.SendClient(FromHex(
        "021F007093569B010403020100D65D59D2"
        "E57792A13E8EDF78A574B8F81D0D3CF0"
        "0003426164"));
    CHECK(mismatch.server->GetState() == warden::WardenState::ModuleReady);
    CHECK(mismatch.server->GetFailure() == warden::WardenFailure::None);
...
TEST(WardenServer_malformed_lua_result_publishes_no_partial_evidence)
{
    ...
    harness.SendClient(FromHex(
        "021B00ABA56906010403020100D65D59D2"
        "E57792A13E8EDF78A574B8F81D0D3CF002"));
    CHECK(harness.server->GetState() == warden::WardenState::Failed);
    CHECK(harness.server->GetFailure() ==
        warden::WardenFailure::MalformedPayload);
...
TEST(WardenServer_timing_timeout_malformed_unexpected_and_send_fail_are_terminal)
{
    ...
    Harness sendFailure(true, TestChecks(8606, "enUS", {65536, 1}));
